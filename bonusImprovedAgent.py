...
def bonusImprovedBelief(matrix, beliefState, targetLocation, initial):
    ...
    while targetFound == False:
        ...
        if withinFive == True:
            ...
            beliefLen = len(belief)

            nextPath = bfs(belief, previousSearch, searching , beliefLen)
            searchingX = searching[0]
            searchingY = searching[1]
            listToNext= traversePath(nextPath, belief, searchingY, searchingX)

            for path in range(len(listToNext)):

                if listToNext[path] == targetLocation:
